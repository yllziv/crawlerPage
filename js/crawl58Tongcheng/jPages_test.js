...
function init() {
    ...
    $.post("http://202.114.114.34:8878/yuqing/servlet_simple_information?query_type=0&seller_type=0&product_type=0&start_time=2015-6-14&end_time=2015-6-15", function (summaryRawData) {//概要页面数据
        $('#shclProgress').hide();
        $("#summaryList").empty();
        ...
        $.post("http://202.114.114.34:8878/yuqing/servlet_detail_information?user_name=%E6%9C%B1%E7%BB%8F%E7%90%86&user_id=48461690", function (detailRawData) {//初始详细页面数据
            $('#detailProgress').hide();
            var detailData = JSON.parse(detailRawData);
            for (var i = 1; i < detailData.length; i++) {
                var a = "<tr><td>" + i + "</td>" +//编号
