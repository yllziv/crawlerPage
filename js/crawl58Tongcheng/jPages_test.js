...
$.post("http://192.168.2.113:8888/yuqing/servlet_simple_information?query_type=0&seller_type=0&product_type=0&start_time=2015-6-10&end_time=2015-6-15", function (summaryRawData) {//概要页面数据
    $("#summaryList").empty();
    ...
    $("#detailList").empty();
    $.post("http://192.168.2.113:8888/yuqing/servlet_detail_information?user_name=%E6%9C%B1%E7%BB%8F%E7%90%86&user_id=48461690", function (detailRawData) {//初始详细页面数据
        var detailData = JSON.parse(detailRawData);
        for (var i = 1; i < detailData.length; i++) {
            var a = "<tr><td>" + i + "</td>" +//编号
                "<td><img src='../images/"+rd(1,20).toString()+".jpg' style='width: 50px;height: 44.5px'></td>" +//图片
